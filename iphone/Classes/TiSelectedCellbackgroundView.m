...
@synthesize rowProxy,fillColor,grouped,position;
...
{
    //NOTE see 5cec3594e7 when rowProxy property was introduced and position property became internal.
    //With introduction of ListView this class became used by both ListView item and TableViewRow.
    //TableViewRow sets rowProxy and expect this class to calculate position, while ListView item sets position by itself.
    //The correct way of doing things is to introduce interface to ask for position and make this class depend on it.
    //But this is quite big refactoring for me at the moment so I keep things as they are and adapting this class by ugly hack.
    //UGLY HACK:If rowProxy is specified then calculate position otherwise use it as it is and expect caller to set it.
    //(averbin)
    if (rowProxy) {
